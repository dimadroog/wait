...
| Аргумент | Описание |
| -------- | -------- |
| `input` | путь к `.fm2` или `YYYYMMDD_playlist.json` |
| `--save-state` | только split-клип: default `inference_cp0.fc0` или sidecar; для embedded FM2 игнорируется |
| `--overlay` | default: `{fm2}.overlay.json` (single FM2) |
| `--no-save-state` | без `-loadstate` (даже для split-клипа) |
| `--turbo` | макс. скорость |

Эфирный лаунчер: `logs/YYYYMMDD_playlist.play.cmd` (генерируется `build_playlist`).

На эфире используйте `play_inference_fm2.py` или `.play.cmd` плейлиста. Self-contained клипы (embedded `savestate` в FM2) проигрываются **без** `-loadstate`; legacy split-клипы (fm2 + `save_state` в manifest/sidecar) — по-прежнему с внешним `.fc0`.
...
**Состав клипа (self-contained, с 3.2):**

| Артефакт | Назначение |
| -------- | ---------- |
| `.fm2` | inputs + embedded `savestate` (gameplay start) |
| `.overlay.json` | achievements overlay для Lua (`achievements`, `stats`, `show_until_frame`); **без** `save_state` |
| `YYYYMMDD_playlist.json` | порядок эфира; `save_state: null` = embedded; иначе путь к `.fc0` (split) |
