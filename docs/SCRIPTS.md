...
Replay требует **embedded** FM2 (savestate в заголовке, `inference_cp0` @ gameplay_start_frame из manifest). Lua: `achievement_overlay_movie.lua` (FM2 `-playmovie`); Python передаёт `WAIT_ACHIEVEMENT_OVERLAY` и `WAIT_BLOCK_LABEL` (плейлист). Без CLI `-loadstate`.
