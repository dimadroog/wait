...
Единица сюжета эфира — [эпизод поколения](#эпизод-поколения); накопленные попытки модели — [пул поколения](#пул-поколения).

### Граница прогресса (frontier)

**Граница прогресса** (frontier) — линия фронта обучения для текущего [поколения](#поколение-модели-genn): самый дальний устойчивый [CP](#cp) и/или кластер смертей (`death_room`, `death_x`), где политика стабильно упирается. На эфире live ведётся вокруг границы. Не путать с [CP](#cp) как узлом маршрута и с номером `genN`.
...
**Inference** (вывод / прогон модели) — режим, в котором уже обученная сеть только **играет**: вызывается `model.predict()`, веса **не** обновляются. Скрипт входа — `run_inference.py` / `inference_local.sh`. Два сценария: **pool** (`--episodes N` → [пул поколения](#пул-поколения)) и **live** (`--live`: окно до Ctrl+C **без** записи пула). Эфир — [STREAMING_CONCEPT.md](STREAMING_CONCEPT.md).

Пул логов — [пул поколения](#пул-поколения). Длина Twitch-слота задаётся оператором (live).
...
2. **Open Broadcaster Software** (программа для стрима) — захват окна [FCEUX](#fceux) (live / опц. replay `.fm2`), кодирование ([NVENC](#nvenc)) и отправка на Twitch.
...
**Эпизод поколения** — единица сюжета эфира: показ возможностей [поколения](#поколение-модели-genn) (`genN`), обычно с контрастом к `genN−1` и фокусом на [границе](#граница-прогресса-frontier). Формат этапа B — live-inference (+ OBS). Не путать с календарным «стримом за день» и с игровым эпизодом внутри env. См. [STREAMING_CONCEPT.md](STREAMING_CONCEPT.md).
