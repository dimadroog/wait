...
1. **Observation** (наблюдение для обучения) — то, что «видит» нейросеть: стек из четырёх кадров размера 112×112 в градациях серого, форма `(4, 112, 112)`. Кадры снимаются из [FCEUX](#fceux) через мост (`obs_*.raw` и декодирование). Не путать с сырыми адресами [RAM](#ram).
