...
Если `--bc-epochs > 0`, нет `--no-bc` и есть `seg_*.npz` с реальными кадрами, `bc_pretrain` несколько эпох минимизирует CrossEntropy по выходам актора, затем стартует обычный PPO. При **continue** того же `model-out` (добор поколения) ветка клонирования пропускается; при **from_ancestor** / **scratch** BC выполняется по флагам.
