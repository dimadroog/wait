...
| 2     | **[3.0]** gameplay save state после intro           | done   | `inference_cp0` = кадр **1250** (G0; не title @18)                                              |
...
| **3.5** | Inference replay: bridge playback (B-proto)                 | **закрыт (B0 FAIL)** | 3.4, ISSUE § B-proto                                         | —                              |
| **3.6** | Inference replay: FM2 gameplay capture (F-proto)            | **done**  | 3.4, ISSUE § G0 / C                                          | —                              |
...
**Статус:** done  
**Вердикт (2026-07-09, уточнён G0 2026-07-16):** gameplay start = кадр **1250** (не 18: `room=0x00,x=129,lives=0` там — title). `scripts/build_inference_states.py` → `states/inference_cp0.fc0`; manifest `inference.gameplay_start_frame`; `run_inference` default — `inference_cp0`. Heuristics — `games/rushn_attack/etalon_build.yaml` + `src/playthrough_build.py` (lives + movement input).
**Этап:** 3.0  
**Приоритет:** high  
...
**Статус:** **закрыт (артефакты)** / **replay FAIL** — jsonl + playlist готовы; standalone GUI эфир **не подтверждён** (title @ f≈28). Replay-слой → **[3.6](#36-inference-replay-fm2-gameplay-capture-f-proto)** § F-proto.  
**Этап:** 3.4  
**Приоритет:** **high**  
...
- [x] **Ручной демо-прогон:** `20260716_playlist.play.cmd` — **FAIL** (оператор: title @ f≈28); superseded by **3.6** (FM2)

### Критерий готовности

- [x] Плейлист из `run_inference` проигрывается end-to-end (автотест + timing)
- [x] ~~На экране FCEUX GUI gameplay с первого кадра~~ — standalone **FAIL**; gate перенесён в **3.6** § F-proto
...
- [x] ~~ISSUE_INFERENCE: закрыть для jsonl replay~~ — standalone отвергнут; issue закрывается в **3.6** § F-proto

### Заметки

- `inference_inputs.jsonl` уже пишется в `run_inference`; retention 4 ч достаточно для сбора плейлиста в тот же день.
- Train / bridge **не трогать** (B-proto закрыт FAIL).
- Не добавлять fallback на movie FM2, «архивный» путь или dual mode.

### Пересмотр (2026-07-16, post-3.4 GUI)

Standalone `play_inference_fm2.py` (CLI `-loadstate` + `achievement_overlay.lua`) **не закрывает эфир**. Путь зафиксирован как FAIL в [ISSUE_INFERENCE.md § BACKLOG 3.4](ISSUE_INFERENCE.md#backlog-34--jsonl-emulation-replay-gui-эфир-2026-07-16). Bridge playback (**3.5**) — тоже FAIL (B0). Дальнейшая работа — **[3.6](#36-inference-replay-fm2-gameplay-capture-f-proto)** § F-proto.

---

## [3.5] Inference replay: bridge playback (B-proto)

**Статус:** **B0 FAIL** (оператор 2026-07-16) — bridge playback не даёт gameplay PPU; **закрыт** (B1/C отменены).  
**Этап:** 3.5  
**Приоритет:** — (закрыт)  
...
Воспроизведение попыток агента на эфире **тем же контрактом**, что запись: `FceuxBridge.reset_to_state(inference_cp0)` + `bridge.step(action)` по `inference_inputs.jsonl`. Gameplay на PPU с кадра 8–28 (визуально, оператор).

### Гипотеза

| ID | Формулировка | Вердикт |
| -- | ------------ | ------- |
| **B0** | Bridge reset + step по jsonl = корректный PPU @ gameplay-start | **отвергнута** (оператор: title → black) |

Контроль: `run_inference` (bridge) — PASS; standalone 3.4 — FAIL (J4).

### Принцип (чистота эксперимента, как N6)

1. **Сначала** изолированный probe вне пайплайна (`tmp/bench/bridge_playback/`).
2. **Gate закрытия фазы** — только **GUI оператор** @ NES frame 8 и 28; headless @ frame 1 **не gate** (P22).
3. **Pass B0+B1** → внедрение в `play_inference_fm2.py` (фаза C).
4. **Fail B0** → зафиксировать отвержение в ISSUE. Pre-render MP4 **не рассматривать** (P25). Целевой эфир — **FM2**; новые гипотезы только FM2/FCEUX-native.

### Вне scope до pass B0

| Компонент | Почему |
| --------- | ------ |
| `play_inference_fm2.py` | меняет критерии до проверки B0 |
| `achievement_overlay.lua` (standalone) | отдельный контракт; overlay — фаза C / OBS |
| `build_playlist` / achievements | артефакты уже есть из 3.4 |
| FM2 / `-playmovie` | снято (N6) |
| Правки `bridge.lua` / train IPC | не лечат эфир; риск регрессии 1.9 |

### Фазы

| Фаза | Что | Gate |
| ---- | --- | ---- |
| **B0** | `FceuxBridge(show_window=True)`, reset `inference_cp0`, 60× `step("")` | оператор: PPU gameplay @ f=8, 28 |
| **B1** | Тот же bridge, actions из `inference_inputs.jsonl` ep1 | оператор @ f=8, 28; скриншоты в `tmp/bench/bridge_playback/` |
| **B2** | Сверка RAM trajectory с `run_inference` на свежем ep | автоматика + выборочный визуал |
| **C** | Backend в `play_inference_fm2.py`, playlist, overlay (OBS или bridge) | оператор: полный клип плейлиста |

### Чеклист сессии

- [x] B0: harness / pytest `requires_fceux` — reset + empty steps
- [x] B0: оператор GUI @ f=8, 28 — **FAIL**: title @ reset, black @ HOLD (2026-07-16)
- [ ] B1: replay реального jsonl (ep1) через bridge — **не начинать** (B0 fail)
- [ ] B1: оператор GUI @ f=8, 28
- [ ] B2: RAM trajectory vs `run_inference` (опц.)
- [ ] Артефакты: `tmp/bench/bridge_playback/b0_results.json`, скриншоты
- [x] **Fail B0:** записать вердикт в ISSUE § B-proto; **не** merge в master
- [ ] **Pass B0+B1:** заменить backend `play_inference_fm2.py`; GUI smoke плейлиста
- [ ] ISSUE_INFERENCE: закрыть issue; merge ветки в master

### Критерий готовности

- [ ] Bridge replay: PPU gameplay @ f=8 и 28 (оператор)
- [ ] Плейлист `playlist.json` проигрывается через bridge end-to-end
- [ ] Standalone `run_inference_playback` deprecated или удалён
- [ ] ISSUE_INFERENCE закрыт
- [ ] Документация: SCRIPTS.md, ML_CONCEPT (replay = bridge, не FM2)

### Заметки

- Train / `fceux_bridge.py` step IPC — **не менять** без отдельной задачи; playback = новый thin wrapper.
- Артефакты bench: `artifact_quarantine_dir("bench", "bridge_playback")`; не писать в `games/.../logs/`.
- При fail B0 — не продолжать J1 (bootstrap в standalone overlay); гипотеза standalone исчерпана. См. **[3.6](#36-inference-replay-fm2-gameplay-capture-f-proto)**.

---

## [3.6] Inference replay: FM2 gameplay capture (F-proto)

**Статус:** **done** — G0 + C1–C3 оператор PASS (2026-07-16). [ISSUE_INFERENCE](ISSUE_INFERENCE.md) **закрыт**.
**Этап:** 3.6
**Приоритет:** —
...
Эфир через **нативный FM2** (`-playmovie` readonly): gameplay на PPU с mf=8–28 (оператор). Embed = `states/inference_cp0.fc0` (кадр 1250, после G0).

### Фазы внедрения (после G0)

| Фаза | Что | Визуальный тест оператора | Статус |
| ---- | --- | ------------------------- | ------ |
| **C1** | `export_fm2` + `--save-episode-fm2`; `play_fm2_gui.py` | один `.fm2` в окне FCEUX | PASS |
| **C2** | `play_inference_fm2.py` → `-playmovie` для `.fm2` | тот же клип + overlay | PASS |
| **C3** | playlist `"fm2"` + `.play.cmd` | плейлист подряд | PASS |

F0/F1/F2 (старый F-proto) — **superseded by G0**.

### Чеклист сессии

- [x] G0: rebuild `inference_cp0` @1250; pytest PPU gameplay @ mf=8
- [x] **C1:** `export_fm2.py` + `run_inference --save-episode-fm2` + `play_fm2_gui.py`
- [x] **C1:** оператор PASS (`play_fm2_gui.py` → `20260716_ep0001.fm2`)
- [x] **C2:** `play_inference_fm2.py` movie path + `achievement_overlay_movie.lua`
- [x] **C2:** оператор PASS (gameplay + overlay на `20260716_ep0001.fm2`)
- [x] **C3:** playlist FM2 + launcher (`playlist.py` → `"fm2"` + `.play.cmd`)
- [x] **C3:** оператор PASS (`20260716_playlist.play.cmd`)
- [x] ISSUE_INFERENCE: закрыт
- [ ] merge `issue/inference-fm2-replay` → master (по запросу)

### Критерий готовности

- [x] FM2 readonly: PPU gameplay (оператор C1–C2)
- [x] Плейлист `playlist.json` через `-playmovie` end-to-end (оператор C3)
- [x] ISSUE_INFERENCE закрыт
- [x] Документация: SCRIPTS.md (replay = FM2-native)

### Заметки

- Корневая причина — ложный `gameplay_start_frame=18` (title); см. ISSUE § G0.
- Embed из `inference_cp0.fc0` на диске — валиден после G0.
