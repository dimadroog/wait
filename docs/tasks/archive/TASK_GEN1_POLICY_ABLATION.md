...
**Итог:** корень провала gen1 — **не PPO**, а **ложные CP на `room 0x00`** (платный старт / фарм награды без реальной игры); канон routes = H3. Серия H0–H5 это подтвердила; побочные noop-эффекты без BC не переносить как объяснение текущих прогонов при уже чистых CP.  
...
**Следствие:** изоляция title/`0x00` от геймплея → [TASK_POLICY_SEPARATION](TASK_POLICY_SEPARATION.md).  
**Приоритет:** high  
...
- Из [GAME_RUSHN_ATTACK §2](../../GAME_RUSHN_ATTACK.md#2-награды-и-чекпоинты-m1) убран развёрнутый ablation/gen1-контекст; в GAME остаётся только фактический канон routes (без ссылки на эту задачу).

### Критерий готовности (DoD)

- [x] Для H0–H5 есть команда, артефакт модели, выписка из логов, `noop_frac`
- [x] В заметках назван **главный** фактор: ложные CP на `0x00` (не PPO); noop-срезы — побочные / confounded
...
**Корень провала (и единственный фактор, который стоит переносить наружу):** ложные CP на `room 0x00` (платный старт + late без `requires_checkpoint`) → залипание `ep_rew_mean≈460` / `ep_len_mean≈11` и `max_checkpoint=4` без реальной игры. Это **не поломка PPO**: при тех же настройках оптимизатора H3 (убрать платный старт + `requires_checkpoint` у late) даёт живую энтропию (~−1.25) и осмысленную награду. Именно фарм title/`0x00` потянул за собой изоляцию фаз → [TASK_POLICY_SEPARATION](TASK_POLICY_SEPARATION.md).

**H0** за +30k не воспроизвёл исходный визуальный «всегда `action=""`» — воспроизвёл **дегенерацию через фарм CP**.

**Побочные наблюдения ряда (не главный вердикт, не цитировать при уже чистых CP):** без BC / слабый BC / фильтр демо / scratch (H1/H2/H4/H5) в том же «плохом» reward-режиме давали высокий `noop_frac`; гипотеза «BC 5 эпох затирает gen0» опровергнута контролем H0 (`noop_frac≈0.02`). Эти noop-срезы описывают старый confounded setup, а не текущий канон routes.

**Рекомендация, зафиксированная тогда для боевого gen1:** routes как H3 (обязательно); тёплый `gen0` + BC ≥ 1…5 — операционный рецепт того момента, не универсальный диагноз «PPO виноват».
