...
Триггер: confirmed deaths до бюджета; secondary — конец **до** или **на** границе game over, не после входа в title/attract. Не голый `x=129`, не слепой trim.
...
- [x] Эталон game over→title→attract: `reference/game_over_to_attract{,2,3,_another_place}.fm2` + ram_scout + визуал; **`y` не инвариант**; game over с другого места уровня → тот же `r=0,x=129`
- [x] Secondary stop на **game-over-freeze** (`r=0,x=129`, `y∉title_ys`, confirm≥32) — код + unit
- [x] **DoD visual:** клип без game over→title→attract (оператор) — **PASS 2026-07-20** (`logs/20260720/ep0001.fm2`, `terminate_reason=game_over_screen`)
- [x] Пост-hoc trim существующих FM2 по game-over-freeze: `logs/20260718` (5/12) + `logs/20260719` (25/25) + refresh `playlist.json` airtime
...
- [x] Исторические плейлисты 18–19.07 обрезаны по game-over-freeze (без title/attract в хвосте)
...
`f=5918` > длины movie (~5788) — хвост attract может быть **hold/free-run** после movie; game over @5031 и title@5714 — **внутри** FM2.

**Выводы (зафиксировать):**

1. Цель **не** достигнута: в клипе есть полная цепочка game over → title → (далее attract).
2. Экран **GAME OVER при `L=6`** — secondary по `lives<1` его **не видит**; `death_count` тоже 0 (нет confirmed death).
3. Стоп по attract standing (`y∈{131,133,135}`, confirm=24) срабатывает **слишком поздно**: к моменту terminate уже записаны game over и title.
4. Нужен эталон/сигнал именно **экрана GAME OVER** (или более ранний конец попытки), не только поза title standing / `lives<1`.
5. Эталон закрыт п. **game-over-etalon** ниже: game over ≠ title standing; **`y=95` не инвариант** (см. game_over_to_attract3 → `y=41`); якорь — **durable freeze**, не `lives`.

Ранее: гейт только `level≥0x08` не включал secondary при gen0 в `room=0` → `min_attempt_steps`; confirm 32→24 убрал «4 цикла» до truncate, но не убрал game over→title из хвоста.

### Эталон game over→title→attract (2026-07-20, вечер)

**Артефакты:** `reference/game_over_to_attract.fm2` (+ `…2`, `…3`, `…_another_place`) — play → game over → title → attract.  
**Scout:** `ram_scout.py … --no-ram-map` на каждый; канон `clear` scout / `ram_resolve` / `ram_map` **не** трогали; dump — `tmp/smoke/game_over_to_attract/` (gitignore).  
**Не** `build_playthrough` — слот `human_playthrough.jsonl` один.

#### Сводка FM2 (кросс-проверка позы)

| FM2 | frames | game over window | game over pose | Title после game over | `L` на game over |
| --- | ------ | --------- | ------- | -------------- | --------- |
| `game_over_to_attract` | 6784 | **2958–3619** (662f) | `r=0 x=129` **`y=95`** freeze | 3642–3753 `y∈{131,133,135}` | **6** |
| `game_over_to_attract2` | 3583 | **2772–3194** (423f) | `r=0 x=129` **`y=95`** freeze | 3204–3328 | **6** |
| `game_over_to_attract3` | 5029 | **4136–4803** (668f) | `r=0 x=129` **`y=41`** freeze | 4829–4923 | **6** |
| `game_over_to_attract_another_place` | 10412 | **9522–10187** (666f) | `r=0 x=129` **`y=41`** freeze | ~10230–10320 | **6** |

**`y=95` отвергнут как единственный якорь:** game_over_to_attract3 / another_place — **`y=41`**.  
**«Другое место уровня»:** до game over герой был при **`x≈8`** (не 129); на экране game over RAM всё равно **`x=129`**. Сигнал game over не зависит от места смерти в коридоре `room=0`.

**Общее на всех game over (4 клипа):**
...
| `0x00EF` | **64** весь game-over-window |
| `0x0001` | **1** почти весь game over (title обычно 0; attract тоже 1) |

**Сигнал game over (кандидат):**

| Критерий | Деталь |
| -------- | ------ |
| Match | `room==0` + `x==129` + `lives≥1` + **`y ∉ {131,133,135}`** + **тот же `(x,y)` держится** |
| Confirm | **≥32** (conf=24 → early false; **32+** — hit только в game over; max non-game-over freeze ≤**31f** на another_place, ранее ≤27) |
| Не делать | фиксировать один `go_y`; голый `x=129` без freeze/confirm |

**Жизни:** на game over→title→attract во всех клипах **`L=6`**. Primary death-budget и secondary `lives<1` этот путь не видят.

#### Операторская визуальная разметка `game_over_to_attract.fm2` (2026-07-20)
...
| **3635** | Title после game over (1 PLAYER) | ≈**−7f** до RAM title-pose **3642–3753** |
| **3805** | Attract/demo (лестница снова) | ≈ RAM attract start **~3810** |

**Следствие:** freeze начинается чуть **раньше** видимого game over; title standing чуть **позже** первого кадра меню. Confirm≥32 на freeze всё ещё попадает в видимый game over (стоп ~f=2989 — уже с текстом GAME OVER на экране).

#### Операторская визуальная разметка `game_over_to_attract2.fm2` (2026-07-20, подробно)

| f (HUD) | На экране | Согласование с RAM |
| ------- | --------- | ------------------ |
| **22** | Title intro | early title |
| **216** | Intro cinema (горы) | не end-attract |
| **1042** | Чёрный экран (fade в уровень) | transition, не game over |
| **1087** | Gameplay start, HUD сверху ещё пустой | вход в уровень |
| **1101** | Gameplay + HUD (`1P`, score, `POW=0`) | обычная игра |
| **2721** | **Death**: герой лежит, враги бегут, HUD жив | **до** game-over-freeze; не стоп-критерий сам по себе |
| **2780** | **GAME OVER** (белый текст, чёрный фон) | внутри RAM-freeze **2772–3194** (`y=95`); текст ≈**+8f** после старта freeze |
| **3224** | Title после game over (1 PLAYER) | внутри RAM title-pose **3204–3328** |
| **3347** | Чёрный экран (fade title→attract) | transition |
| **3391** | Attract/demo (бег у лестницы, HUD) | после title |

**Уточнения относительно game_over_to_attract:**

1. Цепочка визуально: gameplay → **death pose** → game over → title → black → attract.
2. Death @2721 **не** совпадает с началом freeze (2772): ~50f анимации смерти до game-over-экрана.
3. Чёрный экран @1042 / @3347 — **не** GAME OVER (нет текста); нельзя стопить по «весь кадр чёрный».
4. Confirm≥32 от freeze@2772 → стоп ~f=2803 — уже на видимом game over (2780).

#### Операторская визуальная разметка `game_over_to_attract3.fm2` (2026-07-20)

| f (HUD) | На экране | Согласование с RAM |
| ------- | --------- | ------------------ |
| **4137** | **GAME OVER** | RAM-freeze **4136–4803** (`y=41`); текст ≈**+1f** после старта freeze |
| **4823** | Title после game over (1 PLAYER) | ≈**−6f** до RAM title-pose **4829–4923** |
| **4993** | Attract/demo (лестница) | после title; movie ends @5029 |

**Кросс game_over_to_attract{1,2,3}/another_place (визуал ↔ RAM):**

| FM2 | game over text f | Freeze start | Δ | Title f | Attract f |
| --- | ---------------- | ------------ | - | ------- | --------- |
| game_over_to_attract | 2970 | 2958 | +12 | 3635 | 3805 |
| game_over_to_attract2 | 2780 | 2772 | +8 | 3224 | 3391 |
| game_over_to_attract3 | 4137 | 4136 | **+1** | 4823 | 4993 |
| another_place | **9533** | **9522** | +11 | **10213** | **10375** |

Freeze всегда начинается **не позже** видимого game over (Δ 1–12f). Confirm≥32 от start freeze → стоп уже на кадре с текстом GAME OVER во всех клипах.

#### Операторская визуальная разметка `game_over_to_attract_another_place.fm2` (2026-07-20)

| f (HUD) | На экране | Согласование с RAM |
| ------- | --------- | ------------------ |
| **9533** | **GAME OVER** | freeze **9522–10187** (`y=41`); до game over gameplay @ `x≈8` → snap к `x=129` |
| **10213** | Title после game over | переход; title_ys с ~10230 |
| **10375** | Attract/demo (лестница, красная стрелка GO) | после title; movie ends @10412 |

**Вывод another_place:** смерть/обрыв **не у x=129** не ломает детектор — экран game over всё равно даёт `r=0,x=129` + freeze. Отдельный checkpoint (`cp` остался 0) глубже по миссии всё ещё не покрыт, но для коридора `room=0` якорь подтверждён.

**Выводы:**

1. Пробел эталона закрыт **четырьмя** game-over-цепочками (+ визуал) в `reference/`.
2. Ориентир — **durable freeze** на `room=0,x=129` вне title_ys (+ опц. `0x00EF==64`), **не** `lives`, **не** фиксированный `y`, **не** title standing.
3. Visual FAIL: terminate на title_ys слишком поздно; game over уже в FM2.
4. Следующий шаг кода (плагин RnA): secondary `game_over_freeze` confirm≥32 — **сделано** (`terminate_reason=game_over_screen`); DoD visual — оператор.
5. Антискоуп «голый `x=129`» остаётся: нужен freeze+confirm и исключение title_ys.
6. Визуал подтверждает RAM game over ≈ экран GAME OVER; death pose и чёрный fade — не якоря стопа. game over с другого места в `room=0` — тот же сигнал.

#### Inference smoke game_over_freeze (2026-07-20, ночь)

Код: `game_over_freeze_confirm_steps: 32` в `episode_end_title`; `terminate_reason=game_over_screen`.

| | |
|--|--|
| Клип | `tmp/smoke/visual_game_over_freeze/ep0001.fm2` (+ `logs/20260720/ep0001.fm2`) |
| Прогон | `gen0` stochastic, `inference_cp0`, 1 ep |
| steps | **948** env (`fm2` **3792** frames) |
| `terminate_reason` | **`game_over_screen`** |
| `death_count` | **0** |
| end RAM | `r=0x00 x=129 y=95 L=6` (game-over-freeze, не title_ys) |

Play: `./.venv/Scripts/python.exe scripts/play_fm2_gui.py tmp/smoke/visual_game_over_freeze/ep0001.fm2`  
**DoD visual — PASS (оператор 2026-07-20):** клип без game over→title→attract; обрыв на game-over-freeze.
...
**Источники:** `human_playthrough.jsonl` (полного game over нет), dense RAM-probe хвоста `ep0004.fm2` (шаг 30 кадров, последние ~5 с), attempts 18–19.07, G0.
...
| `lives == 0` один кадр | Анимация смерти; не game over |
| `death_x == 129` → trim | Путает death в level с title; см. п.2 |

**Dense хвост raw ep0004 (n=8260)** — title-like **до** logged death, не «после полного game over»:
...
**Пробел эталона (закрыт 2026-07-20 вечером):** полный drain/`lives→0` по-прежнему может отсутствовать; эталон soft-reset game over→title→attract при **`L=6`** — `reference/game_over_to_attract{,2,3,_another_place}.fm2` (game-over-freeze при разном `y`; с другого места уровня — тот же `r=0,x=129`).
...
- При `death_mode=game_over` бюджет может сгорать на flicker → обрыв с `L=5` до настоящего game over; title-like в raw появляется **до** этой смерти (−150…−30), не только «после game over».
- Штатная цепочка после настоящего game over: gameplay → game-over-freeze → title → attract; эталон — `reference/game_over_to_attract{,2,3,_another_place}.fm2` (game over @ `y∈{95,41}`, title @ `y∈{131,133,135}`, всё при `L=6`; another_place: до game over `x≈8`, на game over снова `x=129`).
