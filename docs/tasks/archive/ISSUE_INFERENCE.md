...
**Этап BACKLOG:** 3.4 (jsonl артефакты) · **3.5 закрыт (B0 FAIL)** · **[3.6](TASK_FIRST_CAMPAIGN.md#36-inference-replay-fm2-gameplay-capture-f-proto) закрыт** (FM2-native, оператор C1–C3 PASS)
...
**Связанные документы:** [ML_CONCEPT.md § FM2 из inference](../../ML_CONCEPT.md), [SCRIPTS.md § Inference / Replay](../../SCRIPTS.md), [TASK_FIRST_CAMPAIGN.md § 3.6](TASK_FIRST_CAMPAIGN.md#36-inference-replay-fm2-gameplay-capture-f-proto)
...
Критерий 3.1 («FM2 без `-loadstate`») **не соответствует цели** — плейлист попыток агента на эфире. Movie readonly даёт title при RAM bootstrap. Standalone jsonl (**3.4**) и bridge (**B0**) — FAIL на GUI. Следующий путь → **[BACKLOG 3.6](TASK_FIRST_CAMPAIGN.md#36-inference-replay-fm2-gameplay-capture-f-proto)** § F-proto.
...
- **PPU gameplay** на **GUI эфире** — критерий закрытия issue; активный путь: **§ F-proto** / **[BACKLOG 3.6](TASK_FIRST_CAMPAIGN.md#36-inference-replay-fm2-gameplay-capture-f-proto)**.
...
- **BACKLOG [3.4](TASK_FIRST_CAMPAIGN.md#34-плейлист-попыток-inference-replay)** — артефакты jsonl готовы; standalone replay **FAIL**.
- **BACKLOG [3.5](TASK_FIRST_CAMPAIGN.md#35-inference-replay-bridge-playback-b-proto)** — **закрыт** (B0 FAIL); не продолжать.
- P18 — не приоритет.
...
**Статус:** **закрыт (артефакты)** / **standalone replay FAIL** — jsonl + playlist готовы; GUI title @ f≈28. Визуальный replay → **[BACKLOG 3.6](TASK_FIRST_CAMPAIGN.md#36-inference-replay-fm2-gameplay-capture-f-proto)** § F-proto (FM2-native).
...
| J4 | Replay через standalone FCEUX+Lua ≠ bridge emulation | `run_inference` OK; playlist replay — другой бинарь/путь |
...
**Статус:** **фазы 0–2b + visual sweep выполнены**. Закрытие issue → **§ F-proto** / **[BACKLOG 3.6](TASK_FIRST_CAMPAIGN.md#36-inference-replay-fm2-gameplay-capture-f-proto)**. Harness N6 — диагностика movie mode.
...
N6 фаза 3 (`movie.record` в пайплайне) **не лечит** title @ mf≤50. Standalone jsonl (**3.4**) и bridge (**B0**) — FAIL на GUI. Следующий путь → **§ F-proto** / **[BACKLOG 3.6](TASK_FIRST_CAMPAIGN.md#36-inference-replay-fm2-gameplay-capture-f-proto)**.
...
**BACKLOG:** [3.5](TASK_FIRST_CAMPAIGN.md#35-inference-replay-bridge-playback-b-proto)  
...
**При FAIL B0:** записать наблюдение; не внедрять в пайплайн. Pre-render MP4 **не рассматривать** (P25). Следующий путь → **§ F-proto** / **[BACKLOG 3.6](TASK_FIRST_CAMPAIGN.md#36-inference-replay-fm2-gameplay-capture-f-proto)**.
...
**BACKLOG:** [3.6](TASK_FIRST_CAMPAIGN.md#36-inference-replay-fm2-gameplay-capture-f-proto)  
...
- [x] Issue закрыт = **BACKLOG [3.6](TASK_FIRST_CAMPAIGN.md#36-inference-replay-fm2-gameplay-capture-f-proto)** C1–C3 оператор PASS; ~~3.4 standalone~~ / ~~3.5 bridge~~ — FAIL, не критерий
