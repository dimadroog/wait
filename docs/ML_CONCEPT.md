...
flowchart TB
    subgraph record [Эталон игрока]
        ...
        Demos[games/.../reference/demos_for_bc/seg_*.npz]
        States[games/.../save_states/*.fc*]
        HumanPlay --> Manifest
        HumanPlay --> Demos
        HumanPlay --> States
    end
    ...
    subgraph train [Обучение локально]
        ...
        LocalPPO[PPO на CPU]
        ModelZip[games/.../models/gen0.zip]
        Manifest --> Routes
        States --> LocalPPO
        Routes --> LocalPPO
        LocalPPO --> ModelZip
...
| -------------------------------------- | ----------------------- | ------------------------------------ |
| Save state эмулятора                   | `save_states/cpN.fc*`        | Старт эпизода в FCEUX      |
| Поколение модели | `models/genN.zip` | Веса PPO (в каталоге миссии) |
...
2. При переходе `checkpoint` (0→1, 1→2, …) вызывает `savestate.save("save_states/cpN.fc*")`.
...
```
CACHE save_states/cp2.fc*         → savestate.save(handle) + persist в Lua-кэш (после cold start)
LOAD  save_states/cp2.fc*         → savestate.load(cached handle) + GET_RAM + GET_OBS
...
```
load models/genN.zip
PPO.learn(...), профиль hot_zone
save models/genN+1.zip
```
...
### `reference/demos_for_bc/seg_XXX.npz`
...
| Нарезка эталона                   | `config/playthrough_manifest.yaml`, 3–5 `reference/demos_for_bc/seg_*.npz`                            |
